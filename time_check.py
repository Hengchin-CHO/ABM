...
parser.add_argument('-n', default=2, help='num of landscapes', type=int)
parser.add_argument('-i', default=1, help='iteration for each landscape', type=int)
...
def pre_phase(agents_setting, N, weights, folder, land_number, k, setting, gsize, record):
    ...
    for n in range(N):
        ...
        file = os.path.join('landscape1', setting, f'{setting}_{str(land_number)}.json')
...
def one_game_job(args, record, weights, landscape, setting):
    ...
    folder = os.path.join('landscape1', setting)
...
def select_landscape(g, n, land_l):
    folder = os.path.join('landscape1', f'Grid{g}_Lambda{land_l}')
    folder_contents = os.listdir(folder)
                                ...
                                for struct_name, weights in structures.items():
                                    ...
                                    for j in tqdm(range(n), ncols=70):
                                        ...
                                        for i in range(max_iter):
                                            ...
                                            record_list[i].write()
        print(f"Grid size {g} time: ", time()-starttime)
